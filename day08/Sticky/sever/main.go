...
/*
 * 为什么会出现粘包:
 * 主要原因是tcp数据传递模式是流模式，在保持长连接时可以进行多次收和发
 * "粘包"可能发生在接收端和发送端:
 * 1.由Nagle算法造成的发送端的粘包:Nagle算法是一种改善网络传输效率的算法，当我们提交一段数据给TCP发送时，TCP并不立刻发送此段数据，
...
func process(conn net.Conn) {
	...
	for {
		msg, err := proto.Decoder(reader) // 服务端利用proto包的Decoder解包
		if err == io.EOF {
			return
		}
		if err != nil {
			fmt.Println("decoder msg failed, err:", err)
			return
		}
